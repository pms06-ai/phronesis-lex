...
class ClaimViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        min_certainty = self.request.query_params.get('min_certainty')
        if min_certainty:
            qs = qs.filter(certainty__gte=float(min_certainty))
...
class ContradictionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if same_author is not None:
            qs = qs.filter(same_author=same_author.lower() == 'true')
    ...
    def resolve(self, request, pk=None):
        ...
        contradiction.resolved_by = request.user.username if request.user.is_authenticated else 'anonymous'
        contradiction.save()
...
class CaseContradictionViewSet(viewsets.ViewSet):
    ...
    def detect(self, request, case_pk=None):
        ...
        run = AnalysisRun.objects.create(
            ...
            status='running',
            progress_message='Starting contradiction detection...'
        )
        ...
        try:
            ...
            logger.exception(f"Contradiction detection failed: {e}")
